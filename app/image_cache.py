...
def resize_and_cache(source_path: Path, cache_path: Path, size: int) -> tuple[Path, int, int]:
...
def get_or_create_cached_image(plant: Plant, size: int = 300) -> tuple[Path, int, int]:
    ...
    local_path = Path(plant.image_source_url)
    if local_path.is_file():
        return resize_and_cache(local_path, cache_path, size)
